...
object HelloWorld extends App {
  ...
  println("Does Muffles exist=" + pet_list.exists((pet: Pet) => {
    if (pet.name == "Muffles") true; else false
  }))
  ...
  val dog_list = pet_list.filter((pet: Pet) => {
    if (pet.species == "dog") true; else false
  })
  ...
  assert(two1 == two2)
  // same as equals() method, unlike Java
  ...
  assert(str1.eq(str2))
  // actually checks for same reference, in Java this would be the == operator
