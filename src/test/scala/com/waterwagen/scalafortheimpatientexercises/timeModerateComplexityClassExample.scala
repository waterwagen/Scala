...
class Time(initHours : Int, initMins : Int) {
  if(initHours < 0 || initHours > 23) { throw new IllegalArgumentException("Hours parameter out of range: %d".format(initHours)) }
  if(initMins < 0 || initMins > 59) { throw new IllegalArgumentException("Minutes parameter out of range: %d".format(initMins)) }
